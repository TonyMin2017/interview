...
public class Test {
    ...
    public static void main(String[] args) {
         ...
         * 答：内存溢出是指应用系统中存在无法回收的内存或使用的内存过多，最终使得程序运行要用到的内存大于虚拟机可能提供的最大内存，对于整个应用程序来说，JVM内存空间已经没有多余的空间分配新的对象，所以就发生内存溢出。
         *Java不会内存溢出
         ...
         * 答：ArrayList采用的是数组形式来保存对象的，这种方式将对象放在连续的位置中，所以最大的缺点就是插入或删除时非常慢，在ArrayList的中间插入或删除一个元素意味着这个列表中剩余的元素都会被移动；
         * LinkedList 采用的将对象存放在独立的空间中，而且在每个空间中还保存下一个链接的索引，但是缺点就是查找非常慢要从第一个索引开始，优点是增加、删除快；
